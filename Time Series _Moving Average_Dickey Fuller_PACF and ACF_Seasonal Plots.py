...
MAPE DOESN'T MAKE SENSE IF ONE OBSERVED VALUE IS 0, OR CLOSE TO 0 SINCE IT WILL=INFINITY (FOR STATIONARY DIFFERENCES IT'S OFF),
BUT IT MAKES SENSE IF WE DON'T LOOK AT THE DIFFERENCED DATA
...
for i in tck:
    # for 5 days, you can hardly see a trend, too close to the actual curve
    plot_moving_average_difference(axs[0], stock_hx_data['Adj Close', i], 5, i, plot_intervals=True)
    # Trends over 30 or 90 days are easier to spot
    # Smooth by the previous month (30 days)
    plot_moving_average_difference(axs[1], stock_hx_data['Adj Close', i], 30, i, plot_intervals=True)
    # Smooth by previous quarter (90 days)
    plot_moving_average_difference(axs[2], stock_hx_data['Adj Close', i], 90, i, plot_intervals=True)
    plt.legend(loc="best")
    plt.show()
    fig, axs = plt.subplots(1, 3, figsize=(12, 6), sharex=False, sharey=True)
...
def tsplot(tck, y, lags=None, figsize=(12, 7)):
...
# More about ACF and PACF https://towardsdatascience.com/significance-of-acf-and-pacf-plots-in-time-series-analysis-2fa11a5d10a8
